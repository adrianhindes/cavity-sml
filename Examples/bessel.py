#!/usr/bin/env python
"""-----------------------------------------------------------------
  Python file for plotting Finesse ouput bessel.out
  created automatically Mon Mar  6 11:38:09 2017

  Run from command line as: python bessel.py
  Load from python script as: import bessel
  And then use:
  bessel.run() for plotting only
  x,y=bessel.run() for plotting and loading the data
  x,y=bessel.run(1) for only loading the data
-----------------------------------------------------------------"""
...
def run(noplot=None):
	data = np.loadtxt('bessel.out',comments='%')
	...
	if (noplot==None):
		...
		ax1.set_xlim(0,10)
		ax1.set_xlabel('midx  (eo1)')
		ax1.set_ylabel('Abs ')
		ax1.yaxis.set_major_formatter(formatter)
		trace1=ax1.plot(x, y[:,0], '-', linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'bessel1 n1 : ')
		trace2=ax1.plot(x, y[:,1], '-', linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'bessel2 n1 : ')
		trace3=ax1.plot(x, y[:,2], '-', linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'bessel3 n1 : ')
