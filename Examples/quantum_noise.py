#!/usr/bin/env python
"""-----------------------------------------------------------------
  Python file for plotting Finesse ouput quantum_noise.out
  created automatically Mon Mar  6 11:38:45 2017

  Run from command line as: python quantum_noise.py
  Load from python script as: import quantum_noise
  And then use:
  quantum_noise.run() for plotting only
  x,y=quantum_noise.run() for plotting and loading the data
  x,y=quantum_noise.run(1) for only loading the data
-----------------------------------------------------------------"""
...
def run(noplot=None):
	data = np.loadtxt('quantum_noise.out',comments='%')
	rows,cols=data.shape
	x=data[:,0]
	y=data[:,1:cols]
	mytitle='quantum_noise                Mon Mar  6 11:38:45 2017'
	if (noplot==None):
		...
		trace1=ax1.plot(x, y[:,0], '-', linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'NSR_with_RP nsrc2 : ')
		trace2=ax1.plot(x, y[:,1], '-', linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'NSR_without_RP nsrc2 : ')
