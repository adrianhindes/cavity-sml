#!/usr/bin/env python
"""-----------------------------------------------------------------
  Python file for plotting Finesse ouput 3D.out
  created automatically Mon Mar  6 14:57:55 2017

  Run from command line as: python 3D.py
  Load from python script as: import 3D
  And then use:
  3D.run() for plotting only
  x,y=3D.run() for plotting and loading the data
  x,y=3D.run(1) for only loading the data
-----------------------------------------------------------------"""
...
def run(noplot=None):
	data = np.loadtxt('3D.out',comments='%')
	...
	if (noplot==None):
		...
		surf=ax3.pcolor(xm,ym,z[:,:,0],cmap=cm.jet,linewidth=0, label = 'inphase n3')
