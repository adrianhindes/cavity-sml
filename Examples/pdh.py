#!/usr/bin/env python
"""-----------------------------------------------------------------
  Python file for plotting Finesse ouput pdh.out
  created automatically Mon Mar  6 11:38:34 2017

  Run from command line as: python pdh.py
  Load from python script as: import pdh
  And then use:
  pdh.run() for plotting only
  x,y=pdh.run() for plotting and loading the data
  x,y=pdh.run(1) for only loading the data
-----------------------------------------------------------------"""
...
def run(noplot=None):
	data = np.loadtxt('pdh.out',comments='%')
	...
	if (noplot==None):
		...
		ax1.set_xlabel('f [Hz] (sig1)')
		ax1.set_ylabel('dB ')
		ax1.yaxis.set_major_formatter(formatter)
		ax2 = ax1.twinx()
		trace1=ax1.plot(x, y[:,0], '-', linewidth=pp.LINEWIDTH, color = next(colorcycler), label = ' n3 : dB ')
		trace2=ax2.plot(x, y[:,1], '-', dashes=[8, 4, 2, 4, 2, 4], linewidth=pp.LINEWIDTH, color = next(colorcycler), label = ' n3 : Phase [Deg] ')
