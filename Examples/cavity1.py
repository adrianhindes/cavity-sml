#!/usr/bin/env python
"""-----------------------------------------------------------------
  Python file for plotting Finesse ouput cavity1.out
  created automatically Sat Mar  4 20:41:22 2017

  Run from command line as: python cavity1.py
  Load from python script as: import cavity1
  And then use:
  cavity1.run() for plotting only
  x,y=cavity1.run() for plotting and loading the data
  x,y=cavity1.run(1) for only loading the data
-----------------------------------------------------------------"""
...
def run(noplot=None):
	data = np.loadtxt('cavity1.out',comments='%')
	...
	if (noplot==None):
		...
		ax1.set_xlabel('f [Hz] (i1)')
		ax1.set_ylabel('Abs ')
		ax1.yaxis.set_major_formatter(formatter)
		ax2 = ax1.twinx()
		trace1=ax1.plot(x, y[:,0], '-', linewidth=pp.LINEWIDTH, color = next(colorcycler), label = '  : Abs ')
		trace2=ax2.plot(x, y[:,1], '-', dashes=[8, 4, 2, 4, 2, 4], linewidth=pp.LINEWIDTH, color = next(colorcycler), label = '  : Phase [Deg] ')
