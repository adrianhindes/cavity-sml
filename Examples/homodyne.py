#!/usr/bin/env python
"""-----------------------------------------------------------------
  Python file for plotting Finesse ouput homodyne.out
  created automatically Mon Mar  6 11:38:19 2017

  Run from command line as: python homodyne.py
  Load from python script as: import homodyne
  And then use:
  homodyne.run() for plotting only
  x,y=homodyne.run() for plotting and loading the data
  x,y=homodyne.run(1) for only loading the data
-----------------------------------------------------------------"""
...
def run(noplot=None):
	data = np.loadtxt('homodyne.out',comments='%')
	...
	if (noplot==None):
		...
		ax1.set_xlim(-90,90)
		ax1.set_xlabel('phase [deg] (l1)')
		ax1.set_ylabel('Abs ')
		ax1.yaxis.set_major_formatter(formatter)
		trace1=ax1.plot(x, y[:,0], '-', linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'sqzd_noise nout1 : ')
		trace2=ax1.plot(x, y[:,1], '-', linewidth=pp.LINEWIDTH, color = next(colorcycler), label = 'shot_noise nout1 : ')
